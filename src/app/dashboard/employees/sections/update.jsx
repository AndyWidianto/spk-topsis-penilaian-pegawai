...
import { updateEmployee } from '@/lib/features/employeeSlice';
import { useDispatch } from 'react-redux';

export default function UpdateEmployee({ data, cancel, id }) {
    ...
    const handleSubmit = async () => {
        if (validateForm()) {
            setLoading(true);
            try {
                const res = await fetch(`/api/employees/${id}`, { method: "POST", body: JSON.stringify(formData) });
                const resJson = await res.json();
                console.log(resJson);
                dispatch(updateEmployee(resJson));
    ...
    return (
        <div className="py-12 px-4 sm:px-6 lg:px-8 font-sans">
            <div className="grid grid-cols-1 items-center max-w-2xl mx-auto">
